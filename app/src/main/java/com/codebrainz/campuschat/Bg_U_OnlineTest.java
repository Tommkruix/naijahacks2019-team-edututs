...
public class Bg_U_OnlineTest extends AppCompatActivity {
    //LinearLayout ll1;
   // CircleImageView imageview_unilorin;
   // Dialog dialogPreviewUnilorin;
    ListView listView;
    Bg_U_ListViewAdapter_OnlineTest adapter;
    String[] title;
    String[] description;
    int[] icon;
    ArrayList<Bg_U_Model_OnlineTest> arrayList = new ArrayList<Bg_U_Model_OnlineTest>();
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_bg_u_online_test);
       ...
       /* Toolbar toolbar = findViewById(R.id.toolbar);
        ...
        title = new String[]{"Unilorin"};
        description = new String[]{"Ilorin, Kwara State"};
        icon = new int[]{R.drawable.unilorin_icon, R.drawable.uniabuja_icon, R.drawable.unilag_icon};

        listView = findViewById(R.id.listView);

        for (int i=0; i<title.length; i++){
            Bg_U_Model_OnlineTest model = new Bg_U_Model_OnlineTest(title[i], description[i], icon[i]);
            //bind all strings in an array
            arrayList.add(model);
        }

        //pass resulta to listviewadapter class
        adapter = new Bg_U_ListViewAdapter_OnlineTest(this, arrayList);
   ...
   /* private void showDialogPreviewUnilorin() {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.menu_bg_u_online_test, menu);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_dashboard) {
            Intent i = new Intent(Bg_U_OnlineTest.this, Bg_OnlineTest.class);
            startActivity(i);
        }
